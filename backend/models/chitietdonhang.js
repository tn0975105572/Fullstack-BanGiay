...
chitietdonhang.getById = async (id) => {
  const [rows] = await pool.query("SELECT * FROM chitietdonhang WHERE id = ?", [id]);
  return rows[0];
};
...
chitietdonhang.update = async (id, data) => {
  const [result] = await pool.query("UPDATE chitietdonhang SET ? WHERE id = ?", [data, id]);
  return result.affectedRows;
};

// Xóa chitietdonhang
chitietdonhang.delete = async (id) => {
  const [result] = await pool.query("DELETE FROM chitietdonhang WHERE id = ?", [id]);
  return result.affectedRows;
};
