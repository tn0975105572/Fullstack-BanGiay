...
donhang.getById = async (id) => {
  const [rows] = await pool.query("SELECT * FROM donhang WHERE MaDonHang = ?", [
    id,
  ]);
  return rows[0];
};
...
donhang.update = async (id, data) => {
  const [result] = await pool.query(
    "UPDATE donhang SET ? WHERE MaDonHang = ?",
    [data, id]
  );
...
donhang.delete = async (id) => {
  const [result] = await pool.query("DELETE FROM donhang WHERE MaDonHang = ?", [
    id,
  ]);
  return result.affectedRows;
};
